...
export class BarCodeDecoder {
    ...
    private processImageData(imageData : ImageData) : string | null {
        const TRESHOLD_BLACK_WHITE : number = 127;

        let imageRawData : Uint8ClampedArray = imageData.data;        
        let buffer : Array<number> = [];
    
        for(let i=0; i < imageRawData.length; i+=4) { //RGBA
            let r = imageRawData[i];
            let g = imageRawData[i+1];
            let b = imageRawData[i+2];

            let avg : number = (r+g+b)/3;
            let value : number = avg < TRESHOLD_BLACK_WHITE ? +1 : -1; // +1 => black, -1 => white
            let stripe = buffer[buffer.length - 1];
            ...
            if(!this.isBarcodeValid(buffer)) return null;
        }
    ...
    private decodeDigits(buffer : Array<number>) : string {
        try {
            let module : number = (buffer[1] + Math.abs(buffer[2]) + buffer[3])/3 ;
            let ean13Prefix = '';
            ...
            let barcode = buffer.filter((value : number, index : number) : boolean => {
                ...
                if(index < 6) ean13Prefix += BarCodeDecoder.UPC_SET[value] ? 'L' : 'G';
